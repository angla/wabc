...
public class GameService {
     ...
     * @return
     */
    public static String process(HttpServletRequest request, String openId,
                                 String content) {
        // 获取用户最近一次创建的游戏
        ...
        if (newGameFlag) {
            ...
            // 保存game并获取id
            gameId = MySQLUtil.saveGame(game);
        }
        // 不是新的一局
        else {
        ...
        MySQLUtil.saveGameRound(gameRound);
        // 查询本局游戏的所有回合
        List<GameRound> roundList = MySQLUtil.findAllRoundByGameId(request,
                gameId);
        // 遍历游戏回合，组装给用户回复的消息
        ...
        for (int i = 0; i < roundList.size(); i++) {
            ...
            buffer.append(
                    String.format("第%d回合： %s    %s", i + 1,
                            gameRound.getGuessNumber(),
                            gameRound.getGuessResult())).append("\n");
        }
        // 猜对
        if ("4A0B".equals(guessResult)) {
            ...
            MySQLUtil.updateGame(gameId, 1, GameUtil.getStdDateTime());
        }
        // 10回合仍没猜对
            ...
            buffer.append("重新输入4个不重复的数字开始新的一局。");
            MySQLUtil.updateGame(gameId, 2, GameUtil.getStdDateTime());
        }
        // 本回合没猜对
        else {
            buffer.append("请再接再励！");
    ...
    public static String getUserScore(HttpServletRequest request, String openId) {
        StringBuffer buffer = new StringBuffer();
        HashMap<Integer, Integer> scoreMap = MySQLUtil.getScoreByOpenId(
                request, openId);
        if (null == scoreMap || 0 == scoreMap.size()) {
            ...
            for (Integer status : scoreMap.keySet()) {
                if (0L == status) {
                    buffer.append("游戏中：").append(scoreMap.get(status))
                            .append("\n");
                } else if (1L == status) {
                    buffer.append("胜利局数：").append(scoreMap.get(status))
                            .append("\n");
                } else if (2L == status) {
                    buffer.append("失败局数：").append(scoreMap.get(status))
                            .append("\n");
                }
            }
            buffer.append("请输入4个不重复的数字开始新的一局，例如：0269");
